...
class TestEvidenceItemCreation:
    def test_source_item_from_contextualize(self):
        ...
        called_by = [r for r in relations if r.edge_type == "CALLED_BY"]
    ...
    def test_relation_item_from_transitive(self):
        ...
        assert len(rels) == 1
        assert rels[0].edge_type == "CALLED_BY"
...
class TestLedgerDedup:
    def _make_source(self, eid, node_id, file, start, end, payload=None):
        return EvidenceItem(
            ...
            payload=payload or {"name": node_id.split("::")[-1], "type": "FUNCTION",
    ...
    def test_overlapping_source_range_merges(self):
        ledger = EvidenceLedger()
        ledger.add(self._make_source("a", "src/a.py::foo", "src/a.py", 1, 50))
        # 相同 file + 重叠行号范围
        ledger.add(self._make_source("b", "src/a.py::foo", "src/a.py", 30, 80))
...
class TestDedupKeys:
    def test_source_item_keys(self):
        ...
        assert "node:src/a.py::foo" in keys
        assert "range:src/a.py:1:10" in keys
    ...
    def test_relation_item_keys(self):
        ...
        keys = item.dedup_keys()
        assert "node:src/b.py::bar" in keys
    ...
    def test_no_file_range_key_without_start_line(self):
        ...
        assert not any(k.startswith("range:") for k in keys)
